...
{
    public class AltMacdStrategy : IStrategy
    {
        ...
        public AltMacdStrategy(IOptions<MacdStrategyOptions> options, IIndicatorFactory indicatorFactory)
        {
        ...
        public async Task<TrendDirection> CheckTrendAsync(List<CandleModel> previousCandles, CandleModel currentCandle)
        {
            ...
            else if (_lastTrend == TrendDirection.Long)
            {
                ...
                var stopPercentage = 1 - _macdRate / 100;
                var profitPercentage = 1 + (_macdRate + (decimal)1.5) / 100;
